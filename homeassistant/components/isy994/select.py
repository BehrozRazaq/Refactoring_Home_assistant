...
async def async_setup_entry(
    ...
    for node, control in isy_data.aux_properties[Platform.SELECT]:
        entity = _create_select_entity(node, control, device_info, isy_data)
...
def _create_select_entity(
    ...
    if control == PROP_RAMP_RATE:
        return ISYRampRateSelectEntity(**entity_detail)
    if control == CMD_BACKLIGHT:
        return ISYBacklightSelectEntity(**entity_detail)
    if node.uom == UOM_INDEX and options:
        return ISYAuxControlIndexSelectEntity(**entity_detail)
...
def _get_select_options(node: Node, control: str) -> list[str] | None:
    if control == PROP_RAMP_RATE:
